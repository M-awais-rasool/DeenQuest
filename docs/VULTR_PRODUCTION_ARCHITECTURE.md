...
That cap is a throughput ceiling of roughly 20–30 clips a minute, and no amount of API tuning lifts it. What the API can control is who waits and how: `POST /recitation/check` answers **202** with a job id, workers drain a Redis-backed queue, and the client polls (`internal/recitation/application/jobs.go`). The clip itself is spooled to a TTL-indexed Mongo collection rather than Redis, whose 128 MB is an LRU cache shared with every read model. Two consequences worth knowing: the API's own latency no longer tracks the transcriber's, and a burst past the queue's ceiling is refused with a 429 and a `Retry-After` instead of being accepted and left to time out. Both API colors and both engines share one queue, so a blue/green swap does not lose clips in flight.

**Candidate: replacing the Python service with whisper.cpp.** The prize is memory, not speed. Measured over 58 clips: the Python service peaks at 660 MiB resident, whisper.cpp at 373 MiB — about 290 MB back, not the ~700 MB first estimated, so it covers part of what Mongo's cache and Redis are waiting on rather than all of it. The engines are interchangeable behind `WHISPER_ENGINE` (`faster-whisper` | `whisper-cpp`), running the same Quran-finetuned weights in two formats. The swap is gated on `make whisper-bench`, which compares both on a local clip set and rejects the candidate if any recitation that used to pass now fails — WER alone cannot decide it, because the app shows a score, not a transcript.

One thing the swap gives up: `whisper-server` has no equivalent of the Python service's `X-Internal-Token` check. Choosing it leaves the `internal: true` network as the sole access control on the transcriber. That is plausibly a fair trade for ~290 MB on a network with no route in and no route out — but it is a decision to make deliberately, and the API logs a warning at startup whenever the whisper.cpp engine is selected. Still not decided.

The comparison has now been run, on a reference clip set built from the Quran audio CDN (`bench/fetch-reference-clips.py`): 58 clips, 7 reciters, transcoded to the 16 kHz mono AAC phones upload. whisper.cpp scored **higher** — mean 89.6 against 83.9, WER 18.75% against 23.96% — with no clip that used to pass now failing, and the harness returned ACCEPT. That clears the correctness floor. It does not clear the swap: every voice in that set is a professional reciter, and the gate the README asks for is beginner recordings from real phones, which cannot be downloaded.
