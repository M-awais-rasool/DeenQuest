...
func BuildRules() []NotificationRule {
	return []NotificationRule{
		...
		{
			Type:       QuranSuggestion,
			Cooldown:   24 * time.Hour,
			TimeWindow: TimeWindow{StartHour: 7, EndHour: 11},
			Evaluate: func(ctx *UserContext, now time.Time) bool {
				return true
			},
			BuildTitle: func(ctx *UserContext) string {
		...
		},
		{
			Type:       MulkReminder,
			Cooldown:   24 * time.Hour,
			TimeWindow: TimeWindow{StartHour: 22, EndHour: 23},
			Evaluate: func(ctx *UserContext, now time.Time) bool {
				return true
			},
			BuildTitle: func(ctx *UserContext) string {
...
func pickSuggestedSurah(ctx *UserContext) suggestedSurah {
