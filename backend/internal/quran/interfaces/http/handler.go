...
import (
	...
	"github.com/chawais/deenquest/backend/internal/quran/application"
...
func parseSurahID(c *gin.Context) (int, bool) {
	...
	if err != nil || domain.ValidateSurahID(id) != nil {
...
func writeQuranError(c *gin.Context, err error, fallback string) {
	if errors.Is(err, domain.ErrInvalidSurahID) {
		response.BadRequest(c, "surah id must be between 1 and 114")
		return
	}
	if errors.Is(err, domain.ErrSurahNotFound) {
