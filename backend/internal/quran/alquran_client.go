...
func (c *Client) GetSurahList(ctx context.Context) ([]SurahSummary, error) {
	var envelope apiEnvelope[[]apiSurahSummary]
	if err := c.get(ctx, "/surah", &envelope); err != nil {
		return nil, err
	}

	surahs := make([]SurahSummary, 0, len(envelope.Data))
...
func (c *Client) GetSurahByID(ctx context.Context, id int, translationEdition string) (*SurahDetail, error) {
...
func (c *Client) GetSurahAudio(_ context.Context, id int) (*SurahAudio, error) {
	if err := ValidateSurahID(id); err != nil {
		return nil, err
	}
...
func mapSurahSummary(s apiSurahSummary) SurahSummary {
	return SurahSummary{
		ID:                     s.Number,
		Number:                 s.Number,
		Name:                   s.Name,
...
func mapSurahDetail(s apiSurahDetail) *SurahDetail {
	ayahs := make([]Ayah, 0, len(s.Ayahs))
