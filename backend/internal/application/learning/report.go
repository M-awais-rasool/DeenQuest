...
func NewReportService(l domain.Repository, p progress.CoreRepository) *ReportService {
...
func (s *ReportService) Weekly(ctx context.Context, userID string) (*domain.WeeklyReport, error) {
	now := time.Now().UTC()
	dates := make([]string, 7)
	for i := 0; i < 7; i++ {
	...
	r := &domain.WeeklyReport{UserID: userID, Level: 1, Segment: string(domain.SegmentActive)}
...
func headline(r *domain.WeeklyReport) string {
	switch {
	case r.ActiveDays >= 5:
