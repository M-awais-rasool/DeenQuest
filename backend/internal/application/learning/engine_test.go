...
func TestApplyEvent_WrongAnswersCreateWeakArea(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		ApplyEvent(st, &domain.BehaviorEvent{
			UserID:    "user1",
			Type:      domain.EventAnswerWrong,
			SkillTags: []string{"ت"},
		}, now)
	}
	...
	if st.Segment != domain.SegmentWeak {
...
func TestApplyEvent_CorrectAnswersBuildMasteryAndSchedule(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		ApplyEvent(st, &domain.BehaviorEvent{
			UserID:    "user1",
			Type:      domain.EventAnswerCorrect,
			SkillTags: []string{"ا"},
		}, now)
	}
...
func TestRecommend_WeakDueSkillProducesRevision(t *testing.T) {
	...
	var revision *domain.Recommendation
	for i := range recs {
		if recs[i].Kind == domain.RecRevision {
			revision = &recs[i]
			break
		}
	}
...
func TestApplyEvent_CompletionDoesNotInflateMastery(t *testing.T) {
	...
	ApplyEvent(st, &domain.BehaviorEvent{
		UserID:    "user1",
		Type:      domain.EventLevelCompleted,
		LevelID:   1,
...
func TestRecommend_DueNotStrongSkillProducesRevision(t *testing.T) {
	...
	st.Skills = map[string]domain.SkillStat{
