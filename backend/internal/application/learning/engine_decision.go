...
func Recommend(state *domain.LearnerState, levels []LevelInfo, now time.Time) []domain.Recommendation {
	...
	if state.Segment == domain.SegmentInactive {
		if lv, ok := nextUnlockedIncomplete(levels); ok {
			recs = append(recs, domain.Recommendation{
				Kind:       domain.RecReengage,
				LevelID:    lv.ID,
	...
	for i := range recs {
		...
		recs[i].Status = domain.RecStatusActive
...
func reasonForNewContent(state *domain.LearnerState) string {
	switch state.Segment {
	case domain.SegmentImproving:
		return "Strong recent accuracy — ready for the next step"
	case domain.SegmentWeak:
