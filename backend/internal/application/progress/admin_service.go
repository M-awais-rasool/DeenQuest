...
func (s *CoreService) GetContentRegistry() progress.ContentRegistry {
	return progress.BuildContentRegistry()
}

// ─── Levels ────────────────────────────────────────────────────────────────

func (s *CoreService) AdminListLevels(ctx context.Context) ([]progress.Level, error) {
	return s.repo.ListAllLevels(ctx)
}

func (s *CoreService) AdminGetLevel(ctx context.Context, id int) (*progress.Level, error) {
...
func (s *CoreService) AdminCreateLevel(ctx context.Context, in *progress.Level) (*progress.Level, error) {
	if err := validateLevel(in); err != nil {
		return nil, err
	}
	if in.CourseType == "" {
		in.CourseType = progress.CourseQaida
	}
...
func (s *CoreService) AdminUpdateLevel(ctx context.Context, id int, in *progress.Level) (*progress.Level, error) {
	if err := validateLevel(in); err != nil {
		return nil, err
	}
	in.ID = id
...
func (s *CoreService) AdminListTasks(ctx context.Context) ([]progress.DailyTask, error) {
	return s.repo.ListAllDailyTasks(ctx)
}

func (s *CoreService) AdminGetTask(ctx context.Context, id string) (*progress.DailyTask, error) {
...
func (s *CoreService) AdminCreateTask(ctx context.Context, in *progress.DailyTask) (*progress.DailyTask, error) {
	...
	if in.CompletionType == "" {
		in.CompletionType = progress.CompletionButton
	}
...
func (s *CoreService) AdminUpdateTask(ctx context.Context, id string, in *progress.DailyTask) (*progress.DailyTask, error) {
	if err := validateTask(in); err != nil {
		return nil, err
	}
	in.ID = id
...
func validateLevel(l *progress.Level) error {
...
func validateTask(t *progress.DailyTask) error {
...
func maxLevelID(levels []progress.Level) int {
	max := 0
...
func maxCourseLevel(levels []progress.Level, course progress.CourseType) int {
	max := 0
	for _, l := range levels {
