...
func alignSequences(expected, spoken []string) ([]progress.WordResult, int) {
	...
	origResults := make([]progress.WordResult, 0, E)
	extraResults := make([]progress.WordResult, 0)
	correctCount := 0
	...
	for _, p := range rawPairs {
		...
		case opMatch:
			...
			if p.sim <= threshold {
				origResults = append(origResults, progress.WordResult{
					Text:       expected[p.expIdx],
					Status:     progress.WordCorrect,
		...
		case opDelete:
			origResults = append(origResults, progress.WordResult{
				Text:       expected[p.expIdx],
				Status:     progress.WordMissing,
				Confidence: 0,
			})
		case opInsert:
			extraResults = append(extraResults, progress.WordResult{
				Text:       spoken[p.spkIdx],
				Status:     progress.WordExtra,
				Confidence: 0,
			})
		}
	}
...
func CompareRecitation(expectedText, transcript string) ([]progress.WordResult, int) {
