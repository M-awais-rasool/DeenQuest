...
func (r *MongoCoreRepository) levelsSnapshot(ctx context.Context) ([]Level, error) {
	r.staticMu.RLock()
	if r.levelsLoaded {
...
func (r *MongoCoreRepository) tasksSnapshot(ctx context.Context) ([]DailyTask, error) {
	r.staticMu.RLock()
	if r.tasksLoaded {
		tasks := r.cachedTasks
		r.staticMu.RUnlock()
...
func (r *MongoCoreRepository) rewardsSnapshot(ctx context.Context) ([]Reward, error) {
	r.staticMu.RLock()
	if r.rewardsLoaded {
...
func (r *MongoCoreRepository) GetProgress(ctx context.Context, userID string) (*Progress, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	var p Progress
...
func (r *MongoCoreRepository) UpsertProgress(ctx context.Context, prog *Progress) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) IncrementProgress(ctx context.Context, userID string, xpDelta, barakahDelta int) (*Progress, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) ListLeaderboardProgress(ctx context.Context, limit int) ([]Progress, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) GetStreak(ctx context.Context, userID string) (*Streak, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	var s Streak
...
func (r *MongoCoreRepository) UpsertStreak(ctx context.Context, streak *Streak) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) SeedDailyTasks(ctx context.Context, tasks []DailyTask) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) CreateDailyTask(ctx context.Context, task *DailyTask) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) UpdateDailyTask(ctx context.Context, task *DailyTask) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) ListAllDailyTasks(ctx context.Context) ([]DailyTask, error) {
	return r.tasksSnapshot(ctx)
}

func (r *MongoCoreRepository) GetDailyTaskByID(ctx context.Context, taskID string) (*DailyTask, error) {
...
func (r *MongoCoreRepository) GetUserDailyTasks(ctx context.Context, userID, date string) ([]UserDailyTask, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) UpsertUserDailyTask(ctx context.Context, assignments []UserDailyTask) error {
...
func (r *MongoCoreRepository) SeedLevels(ctx context.Context, levels []Level) error {
	if len(levels) == 0 {
		return nil
	}
...
func (r *MongoCoreRepository) ListAllLevels(ctx context.Context) ([]Level, error) {
...
func (r *MongoCoreRepository) CreateLevel(ctx context.Context, level *Level) error {
...
func (r *MongoCoreRepository) UpdateLevel(ctx context.Context, level *Level) error {
	normalizeLevelDefaults(level)
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func normalizeLevelDefaults(l *Level) {
	if l.CourseType == "" {
		l.CourseType = CourseQaida
	}
	if l.CourseLevel == 0 {
		l.CourseLevel = l.ID
	}
...
func (r *MongoCoreRepository) ListLevelsByCourse(ctx context.Context, courseType CourseType) ([]Level, error) {
	levels, err := r.levelsSnapshot(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *MongoCoreRepository) GetLevelByID(ctx context.Context, levelID int) (*Level, error) {
	levels, err := r.levelsSnapshot(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *MongoCoreRepository) GetNextLevelByCourseLevel(ctx context.Context, courseType CourseType, courseLevel int) (*Level, error) {
	levels, err := r.levelsSnapshot(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *MongoCoreRepository) GetUserLevels(ctx context.Context, userID string) ([]UserLevel, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) GetUserLevelsByLevelIDs(ctx context.Context, userID string, levelIDs []int) ([]UserLevel, error) {
	if len(levelIDs) == 0 {
	...
	defer cur.Close(ctx)
	out := make([]UserLevel, 0, len(levelIDs))
	for cur.Next(ctx) {
		var ul UserLevel
		if err := cur.Decode(&ul); err != nil {
			return nil, err
...
func (r *MongoCoreRepository) GetUserLevel(ctx context.Context, userID string, levelID int) (*UserLevel, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	var ul UserLevel
...
func (r *MongoCoreRepository) UpsertUserLevel(ctx context.Context, ul *UserLevel) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) SeedRewards(ctx context.Context, rewards []Reward) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) ListAllRewards(ctx context.Context) ([]Reward, error) {
	return r.rewardsSnapshot(ctx)
}

func (r *MongoCoreRepository) GetRewardByID(ctx context.Context, id string) (*Reward, error) {
...
func (r *MongoCoreRepository) CreateReward(ctx context.Context, reward *Reward) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) UpdateReward(ctx context.Context, reward *Reward) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) GetUserRewards(ctx context.Context, userID string) ([]UserReward, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) GrantUserReward(ctx context.Context, ur *UserReward) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoCoreRepository) SaveRecitationAttempt(ctx context.Context, attempt *RecitationAttempt) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
