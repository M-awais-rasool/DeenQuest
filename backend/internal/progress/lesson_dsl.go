...
// Lesson DSL — server-side contract of the lesson engine (plan §2.3).
//
// A lesson with Component == EngineComponent carries a DSL document in its
// Data. The app's LessonEngine (DeenQuestExpo app/components/level/lesson/
// engine/) routes it to one of the 9 interaction primitives; this file is the
// backend's matching validator so invalid DSL can never reach a device.
// Keep both sides in sync.

// LessonEngineVersion is the engine capability level this backend authors
// content for. Content may not require a newer engine than this — the app
// would only ever show its fallback card.
const LessonEngineVersion = 1

// EngineComponent is the sentinel component name that routes a lesson through
// the app's LessonEngine.
const EngineComponent = "engine"

// validInteractions — the 9 primitives (plan §2.2), mirror of the app's
// INTERACTIONS list.
...
// ValidateLessonDSL checks one engine lesson's Data document and returns
// every problem found. It reuses the same guard helpers as the plain content
// linter so limits (≤4 options, ≤12 grid cells, timer ≥6s, Arabic-only
// answers) stay identical whichever way a lesson is authored.
...
// validateTeach requires at least one payload shape the app's teach adapter
// can dispatch on.
...
// validateChoice mirrors the app's choice dispatch: timed → lightning rules,
// binary → true/false rounds, audio → listen&choose, no-correct → reflection,
// else MCQ.
