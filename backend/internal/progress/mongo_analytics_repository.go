...
func (r *MongoAnalyticsRepository) GetAdminAnalytics(ctx context.Context) (*AdminAnalytics, error) {
	r.mu.Lock()
...
func (r *MongoAnalyticsRepository) activitySeries(ctx context.Context, start time.Time, startStr string) []AnalyticsTimePoint {
	points := make([]AnalyticsTimePoint, analyticsSeriesDays)
	index := make(map[string]int, analyticsSeriesDays)
	for i := 0; i < analyticsSeriesDays; i++ {
		d := start.AddDate(0, 0, i).Format("2006-01-02")
		points[i] = AnalyticsTimePoint{Date: d}
		index[d] = i
	}
...
func (r *MongoAnalyticsRepository) levelsByDifficulty(ctx context.Context) []AnalyticsLabelCount {
	...
	out := make([]AnalyticsLabelCount, 0, len(rows))
	for _, row := range rows {
		label := row.ID
		if label == "" {
			label = "unset"
		}
		out = append(out, AnalyticsLabelCount{Label: label, Count: row.Count})
	}
	sort.Slice(out, func(i, j int) bool { return out[i].Label < out[j].Label })
	return out
}

func (r *MongoAnalyticsRepository) topLevels(ctx context.Context) []AnalyticsLabelCount {
