...
func (c *Client) GetSurahList(ctx context.Context) ([]quran.SurahSummary, error) {
	var envelope apiEnvelope[[]apiSurahSummary]
	if err := c.get(ctx, "/surah", &envelope); err != nil {
		return nil, err
	}

	surahs := make([]quran.SurahSummary, 0, len(envelope.Data))
...
func (c *Client) GetSurahByID(ctx context.Context, id int, translationEdition string) (*quran.SurahDetail, error) {
	if err := quran.ValidateSurahID(id); err != nil {
		return nil, err
	}
...
func (c *Client) GetSurahAudio(_ context.Context, id int) (*quran.SurahAudio, error) {
	if err := quran.ValidateSurahID(id); err != nil {
		return nil, err
	}

	return &quran.SurahAudio{
		SurahID: id,
...
func mapSurahSummary(s apiSurahSummary) quran.SurahSummary {
	return quran.SurahSummary{
		ID:                     s.Number,
		Number:                 s.Number,
		Name:                   s.Name,
...
func mapSurahDetail(s apiSurahDetail) *quran.SurahDetail {
	ayahs := make([]quran.Ayah, 0, len(s.Ayahs))
	for _, a := range s.Ayahs {
		ayahs = append(ayahs, quran.Ayah{
			Number:        a.Number,
