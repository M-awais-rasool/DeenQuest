...
func (r *MongoMistakeRepository) RecordMistake(ctx context.Context, m learning.Mistake) error {
...
func (r *MongoMistakeRepository) ListMistakes(ctx context.Context, userID string, includeResolved bool, limit int) ([]learning.Mistake, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	...
	defer cur.Close(ctx)
	out := make([]learning.Mistake, 0, 16)
	for cur.Next(ctx) {
		var m learning.Mistake
...
func (r *MongoMistakeRepository) TopMissed(ctx context.Context, limit int) ([]learning.LessonStruggle, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	...
	defer cur.Close(ctx)
	out := make([]learning.LessonStruggle, 0, limit)
	for cur.Next(ctx) {
		var row struct {
			ID struct {
		...
		out = append(out, learning.LessonStruggle{
