...
func (r *MongoUserRepository) Create(ctx context.Context, user *identity.User) error {
...
func (r *MongoUserRepository) GetByEmail(ctx context.Context, email string) (*identity.User, error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	var u identity.User
...
func (r *MongoUserRepository) GetByID(ctx context.Context, id string) (*identity.User, error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	var u identity.User
...
func (r *MongoUserRepository) Update(ctx context.Context, user *identity.User) error {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
