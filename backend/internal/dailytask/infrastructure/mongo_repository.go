...
func (r *MongoRepository) tasksSnapshot(ctx context.Context) ([]domain.DailyTask, error) {
	r.staticMu.RLock()
	if r.tasksLoaded {
		tasks := r.cachedTasks
		r.staticMu.RUnlock()
	...
	defer cur.Close(ctx)
	out := make([]domain.DailyTask, 0, 16)
	for cur.Next(ctx) {
		var t domain.DailyTask
...
func (r *MongoRepository) SeedDailyTasks(ctx context.Context, tasks []domain.DailyTask) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoRepository) CreateDailyTask(ctx context.Context, task *domain.DailyTask) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoRepository) UpdateDailyTask(ctx context.Context, task *domain.DailyTask) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoRepository) ListAllDailyTasks(ctx context.Context) ([]domain.DailyTask, error) {
	return r.tasksSnapshot(ctx)
}

func (r *MongoRepository) GetDailyTaskByID(ctx context.Context, taskID string) (*domain.DailyTask, error) {
...
func (r *MongoRepository) GetUserDailyTasks(ctx context.Context, userID, date string) ([]domain.UserDailyTask, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	...
	defer cur.Close(ctx)
	out := make([]domain.UserDailyTask, 0, 5)
	for cur.Next(ctx) {
		var ut domain.UserDailyTask
...
func (r *MongoRepository) UpsertUserDailyTask(ctx context.Context, assignments []domain.UserDailyTask) error {
