...
func (r *MongoRepository) StoreEvents(ctx context.Context, events []domain.StoredEvent) error {
	if len(events) == 0 {
		return nil
	}
...
func (r *MongoRepository) GetSkillState(ctx context.Context, userID string) (*domain.UserSkillState, error) {
	var state domain.UserSkillState
...
func (r *MongoRepository) SaveSkillState(ctx context.Context, state *domain.UserSkillState) error {
...
func (r *MongoRepository) UpsertInsights(ctx context.Context, insights []domain.Insight) error {
	if len(insights) == 0 {
		return nil
	}
...
func (r *MongoRepository) ExpireInsightsNotIn(ctx context.Context, userID string, activeIDs []string, now time.Time) error {
	...
	_, err := r.insights.UpdateMany(ctx,
		bson.M{"user_id": userID, "status": domain.InsightActive, "_id": bson.M{"$nin": activeIDs}},
		bson.M{"$set": bson.M{"status": domain.InsightExpired, "updated_at": now}})
	if err != nil {
...
func (r *MongoRepository) ActiveInsights(ctx context.Context, userID string, now time.Time) ([]domain.Insight, error) {
	cur, err := r.insights.Find(ctx,
		bson.M{"user_id": userID, "status": domain.InsightActive, "expires_at": bson.M{"$gt": now}},
...
func (r *MongoRepository) GetInsight(ctx context.Context, userID, insightID string) (*domain.Insight, error) {
...
func (r *MongoRepository) MarkInsightDone(ctx context.Context, userID, insightID string) error {
	_, err := r.insights.UpdateOne(ctx,
		...
		bson.M{"$set": bson.M{"status": domain.InsightDone, "updated_at": time.Now()}})
	if err != nil {
...
func (r *MongoRepository) EachSkillState(ctx context.Context, fn func(*domain.UserSkillState) error) error {
