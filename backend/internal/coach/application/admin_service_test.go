...
func (f *fakeAdminRepo) EachSkillState(_ context.Context, fn func(*domain.UserSkillState) error) error {
	if f.failNow != nil {
		return f.failNow
	}
	f.scans++
...
func newFakeAdminRepo() *fakeAdminRepo {
	weak := domain.NewUserSkillState("weak-learner")
	weak.Skills["ب"] = &domain.SkillStat{Attempts: 20, Correct: 4, EMAAccuracy: 0.2, LastSeen: adminNow}

	lapsed := domain.NewUserSkillState("lapsed-learner")
	lapsed.Skills["ت"] = &domain.SkillStat{
		Attempts: 30, Correct: 27, EMAAccuracy: 0.9,
		LastSeen: adminNow.AddDate(0, 0, -inactiveDays-2),
	}

	return &fakeAdminRepo{
		states: []*domain.UserSkillState{weak, lapsed},
		active: 12,
		decay:  4,
		events: 9001,
		missed: []LessonStruggle{
