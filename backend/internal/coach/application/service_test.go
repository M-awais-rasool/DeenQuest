...
// fakeRepo is an in-memory domain.Repository; mutexed because Ingest evaluates rules
// in a background goroutine.
type fakeRepo struct {
	mu       sync.Mutex
	batches  map[string]bool
	events   []domain.StoredEvent
	states   map[string]*domain.UserSkillState
...
func (f *fakeRepo) StoreEvents(_ context.Context, events []domain.StoredEvent) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func cloneState(s *domain.UserSkillState) *domain.UserSkillState {
	if s == nil {
		return nil
	}
	cp := *s
	cp.Skills = make(map[string]*domain.SkillStat, len(s.Skills))
	...
	cp.Days = make(map[string]*domain.DayStat, len(s.Days))
...
func (f *fakeRepo) GetSkillState(_ context.Context, userID string) (*domain.UserSkillState, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	return cloneState(f.states[userID]), nil
}

func (f *fakeRepo) SaveSkillState(_ context.Context, state *domain.UserSkillState) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeRepo) UpsertInsights(_ context.Context, insights []domain.Insight) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	for _, ins := range insights {
		...
		ins.Status = domain.InsightActive
...
func (f *fakeRepo) ExpireInsightsNotIn(_ context.Context, userID string, activeIDs []string, now time.Time) error {
	...
	for id, ins := range f.insights {
		if ins.UserID == userID && ins.Status == domain.InsightActive && !keep[id] {
			ins.Status = domain.InsightExpired
		}
	}
	return nil
}

func (f *fakeRepo) ActiveInsights(_ context.Context, userID string, now time.Time) ([]domain.Insight, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	var out []domain.Insight
	for _, ins := range f.insights {
		if ins.UserID == userID && ins.Status == domain.InsightActive && ins.ExpiresAt.After(now) {
			out = append(out, *ins)
		}
	}
	return out, nil
...
func (f *fakeRepo) GetInsight(_ context.Context, userID, insightID string) (*domain.Insight, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeRepo) EachSkillState(_ context.Context, fn func(*domain.UserSkillState) error) error {
	f.mu.Lock()
	states := make([]*domain.UserSkillState, 0, len(f.states))
...
func TestIngestIdempotency(t *testing.T) {
	...
	events := []domain.TelemetryEvent{answerEvent("ت", true, testNow)}
...
func TestIngestDropsStaleEvents(t *testing.T) {
	...
	n, err := svc.Ingest(context.Background(), "u1", "", []domain.TelemetryEvent{stale, future, fresh})
	if err != nil || n != 1 {
...
func TestThreeMistakesProduceInsight(t *testing.T) {
	...
	if ins.Severity != domain.SeverityHigh || ins.PracticeLevelID == 0 {
...
func TestPracticeFlowAwardsXPOnce(t *testing.T) {
	...
	if lvl.ID != domain.PairPracticeID("ت", "ث") || len(lvl.Lessons) != 4 {
		t.Errorf("practice level = id %d with %d lessons", lvl.ID, len(lvl.Lessons))
	}

	xp, err := svc.CompletePractice(ctx, "u1", insightID)
	if err != nil || xp != domain.PracticeXP {
		t.Fatalf("complete = (%d, %v), want (%d, nil)", xp, err, domain.PracticeXP)
	}
	xp, err = svc.CompletePractice(ctx, "u1", insightID)
	if err != nil || xp != 0 {
...
func TestCompletePracticeStaysDoneAcrossReEvaluation(t *testing.T) {
	...
	insightID := domain.InsightID("u1", domain.RuleConfusionPair, []string{"ص", "ض"})
	...
	_, _ = svc.Ingest(ctx, "u1", "b2", []domain.TelemetryEvent{correct})
	_ = svc.EvaluateUser(ctx, "u1")
	...
	var fresh []domain.TelemetryEvent
	for i := 0; i < 3; i++ {
