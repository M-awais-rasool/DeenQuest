...
func MeanMastery(state *domain.UserSkillState, now time.Time) float64 {
	if state == nil || len(state.Skills) == 0 {
		return 0
	}
	sum := 0.0
	for _, s := range state.Skills {
		sum += domain.Mastery(s, now)
	}
	return sum / float64(len(state.Skills))
}

func LastActive(state *domain.UserSkillState) time.Time {
	var last time.Time
	if state == nil {
		return last
	}
...
func DaysSinceActive(state *domain.UserSkillState, now time.Time) float64 {
...
func EngagementScore(state *domain.UserSkillState, now time.Time) float64 {
	if state == nil || len(state.Days) == 0 {
		return 0
	}
	active := 0
	for i := 0; i < engagementWindow; i++ {
		key := domain.DayKey(now.AddDate(0, 0, -i))
...
func DropoutRisk(state *domain.UserSkillState, now time.Time) float64 {
...
func SegmentOf(state *domain.UserSkillState, now time.Time) string {
	...
	if _, delta := domain.WeekAccuracy(state, now); delta > 0 {
...
func (f *statsFold) add(state *domain.UserSkillState, now time.Time) {
	if state == nil {
		return
	}
	f.learners++
