...
func (p *Phraser) Detail(ctx context.Context, userID string, ins domain.Insight) string {
	if p == nil || !p.enabled {
		return ins.Detail
	}
...
func HomeMessage(ins domain.Insight) domain.MessageParts {
	switch ins.Rule {
	case domain.RuleConfusionPair:
		if len(ins.Skills) == 2 {
			return domain.MessageParts{
	...
	return domain.MessageParts{Before: ins.Title + " — ", Highlight: ins.Detail, Tail: ""}
}
