...
func CompilePairPractice(a, b string) (*leveldomain.Level, error) {
	...
	lessons := []leveldomain.Lesson{
		{
			Type: leveldomain.LessonQaida, Title: "Look Closely",
			Description: fmt.Sprintf("Two letters, one difference — %s vs %s", a, b),
			ScreenType:  leveldomain.ScreenAction, Component: "LetterIntroComponent",
		...
		{
			Type: leveldomain.LessonQuiz, Title: "Pick the Right One",
			Description: "Choose carefully — the coach is watching",
			ScreenType:  leveldomain.ScreenQuiz, Component: "MCQComponent",
			SkillTags: []string{a, b},
			Data: map[string]any{
				"questions": []any{
		...
		{
			Type: leveldomain.LessonQaida, Title: "Letter Hunt",
			Description: fmt.Sprintf("Find every %s — ignore the lookalikes", lb.Name),
			ScreenType:  leveldomain.ScreenAction, Component: "LetterHuntComponent",
			SkillTags: []string{b},
			Data: map[string]any{
		...
		{
			Type: leveldomain.LessonRevision, Title: "Lightning Review",
			Description: "Fast rounds — trust your eyes",
			ScreenType:  leveldomain.ScreenQuiz, Component: "LightningRoundComponent",
			SkillTags: []string{a, b},
			Data: map[string]any{
				"seconds": 6,
				"questions": []any{
