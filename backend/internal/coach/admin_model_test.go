...
func stateWith(ema float64, daysAgo int) *UserSkillState {
	s := NewUserSkillState("u1")
	s.Skills["ت"] = &SkillStat{
		Attempts:    20,
		Correct:     int(ema * 20),
		EMAAccuracy: ema,
...
func TestSegmentOfWeakBeatsImproving(t *testing.T) {
	...
	s := stateWith(0.3, 0)
	monday := startOfISOWeek(adminNow)
	s.Days[dayKey(monday)] = &DayStat{Attempts: 10, Correct: 8}
	s.Days[dayKey(monday.AddDate(0, 0, -7))] = &DayStat{Attempts: 10, Correct: 2}
...
func TestSegmentOfImprovingWhenTrendingUp(t *testing.T) {
	...
	monday := startOfISOWeek(adminNow)
	s.Days[dayKey(monday)] = &DayStat{Attempts: 10, Correct: 9}
	s.Days[dayKey(monday.AddDate(0, 0, -7))] = &DayStat{Attempts: 10, Correct: 4}
...
func TestEngagementScoreCountsDistinctDays(t *testing.T) {
	s := NewUserSkillState("u1")
	for i := 0; i < 3; i++ {
		s.Days[dayKey(adminNow.AddDate(0, 0, -i))] = &DayStat{Attempts: 4, Correct: 3}
	}
	// A day outside the 7-day window must not count.
	s.Days[dayKey(adminNow.AddDate(0, 0, -20))] = &DayStat{Attempts: 9, Correct: 9}
...
func TestDropoutRiskSaturates(t *testing.T) {
	...
	if got := DropoutRisk(NewUserSkillState("u1"), adminNow); got != 1 {
...
func TestWeakestSkillsRanksAndAverages(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		s := NewUserSkillState("u")
		s.Skills["ب"] = &SkillStat{EMAAccuracy: 0.2, LastSeen: adminNow}
		f.add(s, adminNow)
	}
	strong := NewUserSkillState("u3")
	strong.Skills["ت"] = &SkillStat{EMAAccuracy: 0.95, LastSeen: adminNow}
