...
// XPAwarder is the slice of progress.Service the coach needs to grant XP for
type XPAwarder interface {
	Award(ctx context.Context, userID string, xpDelta, barakahDelta int) (*progress.Progress, error)
}
...
func NewService(repo Repository, awarder XPAwarder, phraser *Phraser) *Service {
	return &Service{repo: repo, progress: awarder, phraser: phraser, now: time.Now}
}

// --- ingest ----------------------------------------------------------------
func (s *Service) Ingest(ctx context.Context, userID, idempotencyKey string, events []TelemetryEvent) (int, error) {
	if len(events) == 0 {
		return 0, nil
	}
	...
	now := s.now()
	accepted := make([]TelemetryEvent, 0, len(events))
	for _, ev := range events {
		if ev.Type == "" || ev.TS <= 0 {
			continue
		}
		at := ev.Time()
		if now.Sub(at) > MaxEventAge || at.After(now.Add(5*time.Minute)) {
	...
	stored := make([]StoredEvent, len(accepted))
	...
	if state == nil {
		state = NewUserSkillState(userID)
	}
...
func (s *Service) reconcileInsights(ctx context.Context, state *UserSkillState, now time.Time) error {
	insights := EvaluateRules(state, now)
...
func (s *Service) CoachState(ctx context.Context, userID string) (*CoachStateDTO, error) {
	state, err := s.repo.GetSkillState(ctx, userID)
	if err != nil {
		return nil, err
	}
	...
	week, weekDelta := WeekAccuracy(state, now)
	lessons := LessonsInWindow(state, now, 7)
...
func (s *Service) insightDTO(ctx context.Context, userID string, ins Insight) InsightDTO {
...
func insightGlyph(ins Insight) (glyph string, isArabic bool) {
	if len(ins.Skills) == 2 && ContainsArabic(ins.Skills[0]) && ContainsArabic(ins.Skills[1]) {
		// Zero-width non-joiner keeps the two letters in isolated form.
		return ins.Skills[0] + "‌" + ins.Skills[1], true
	}
	if len(ins.Skills) >= 1 && ContainsArabic(ins.Skills[0]) {
...
func missionTitle(ins Insight) string {
...
// --- practice --------------------------------------------------------------
func (s *Service) Practice(ctx context.Context, userID, insightID string) (*level.Level, error) {
	...
	a, b, ok := PairFromPracticeID(ins.PracticeLevelID)
	if !ok {
		return nil, ErrNoPractice
	}
	return CompilePairPractice(a, b)
}
...
func (s *Service) SweepAll(ctx context.Context) (users int, err error) {
	err = s.repo.EachSkillState(ctx, func(state *UserSkillState) error {
