...
var (
	...
	ErrNoToken      = errors.New("user has no notification token")
	ErrBadMessage   = errors.New("notification message needs title or body")
)
...
func NewService(tokens TokenRepository, sender Sender) *Service {
	return &Service{tokens: tokens, sender: sender}
}

func (s *Service) RegisterToken(ctx context.Context, user UserInfo, req RegisterTokenRequest) (*TokenResponse, error) {
	expoPushToken := strings.TrimSpace(req.ExpoPushToken)
	if !push.IsExpoPushToken(expoPushToken) {
		return nil, ErrInvalidToken
	}

	saved, err := s.tokens.Upsert(ctx, &UserToken{
		User:          user,
