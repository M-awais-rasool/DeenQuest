...
// MongoStateRepository persists LearnerState (learner_states) and
...
func NewMongoStateRepository(db *mongo.Database) (*MongoStateRepository, error) {
	r := &MongoStateRepository{
...
func (r *MongoStateRepository) ensureIndexes() error {
...
func (r *MongoStateRepository) GetState(ctx context.Context, userID string) (*model.LearnerState, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	var s model.LearnerState
...
func (r *MongoStateRepository) UpsertState(ctx context.Context, state *model.LearnerState) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
...
func (r *MongoStateRepository) ListStates(ctx context.Context, limit, offset int) ([]model.LearnerState, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	...
	defer cur.Close(ctx)
	out := make([]model.LearnerState, 0, 64)
	for cur.Next(ctx) {
		var s model.LearnerState
...
func (r *MongoStateRepository) ListSweepCandidates(ctx context.Context, inactiveBefore, dueBefore time.Time, limit, offset int) ([]model.LearnerState, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()

	filter := bson.M{"$or": []bson.M{
		{"last_event_at": bson.M{"$lt": inactiveBefore}, "segment": bson.M{"$ne": string(model.SegmentInactive)}},
...
func (r *MongoStateRepository) ReplaceActiveRecommendations(ctx context.Context, userID string, recs []model.Recommendation) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()

	if _, err := r.recommendations.DeleteMany(
		timeoutCtx,
		bson.M{"user_id": userID, "status": model.RecStatusActive},
	); err != nil {
		return err
	}
...
func (r *MongoStateRepository) ListActiveRecommendations(ctx context.Context, userID string) ([]model.Recommendation, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	...
	defer cur.Close(ctx)
	out := make([]model.Recommendation, 0, 8)
	for cur.Next(ctx) {
...
func (r *MongoStateRepository) Stats(ctx context.Context, now time.Time) (*model.AgentStats, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	...
	stats := &model.AgentStats{Segments: map[string]int{}}
	if len(raw) > 0 {
		f := raw[0]
	...
	activeCount, err := r.recommendations.CountDocuments(timeoutCtx, bson.M{"status": model.RecStatusActive})
	if err != nil {
		return nil, err
	}
...
func (r *MongoStateRepository) SkillStruggles(ctx context.Context, limit int) ([]model.SkillStruggle, error) {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	...
	defer cur.Close(ctx)
	out := make([]model.SkillStruggle, 0, limit)
	for cur.Next(ctx) {
		var row struct {
			ID         string  `bson:"_id"`
		...
		out = append(out, model.SkillStruggle{
			Tag:          row.ID,
...
func (r *MongoStateRepository) SetMotivation(ctx context.Context, userID, message string) error {
	timeoutCtx, cancel := withTimeout(ctx)
	defer cancel()
	_, err := r.states.UpdateOne(
		timeoutCtx,
