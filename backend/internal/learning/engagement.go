...
func NewEngagementNotifier(repo model.Repository, sender ReengageSender, gen Generator) *EngagementNotifier {
...
func (n *EngagementNotifier) NotifyReengage(ctx context.Context, userID string) {
	...
	_, err := n.sender.SendToUser(ctx, notification.UserInfo{ID: userID}, notification.Message{
		Title: title,
		Body:  body,
...
func (n *EngagementNotifier) message(ctx context.Context, state *model.LearnerState) (title, body string) {
	title = "We miss you"
...
func topWeak(state *model.LearnerState, n int) string {
