...
func NewAIService(repo model.Repository, gen Generator) *AIService {
...
// Handle is a kafka.MessageHandler. Only a few event types are narrated to keep
// cost down; everything else is ignored.
func (s *AIService) Handle(ctx context.Context, event kafka.Event) error {
...
func (s *AIService) promptFor(ctx context.Context, ev *model.BehaviorEvent) (string, bool) {
	switch ev.Type {
