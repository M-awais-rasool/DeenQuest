...
func parseOptionalCourseType(raw string) (domain.CourseType, bool) {
	if raw == "" {
		return "", true
	}
	return domain.CourseTypeFromString(raw)
}

func (h *Handler) GetLevels(c *gin.Context) {
	userID := c.GetString("user_id")
	courseType, ok := domain.CourseTypeOrDefault(c.Query("course_type"))
	if !ok {
...
func (h *Handler) CompleteLesson(c *gin.Context) {
	...
	if err != nil {
		...
		if errors.Is(err, application.ErrInvalidLessonIndex) {
