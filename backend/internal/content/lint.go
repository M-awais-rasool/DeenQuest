...
var knownMiniGames = map[level.MiniGameType]bool{
	level.GameTapMatch:     true,
	level.GameListenChoose: true,
	level.GameDragDrop:     true,
	level.GameRepeatVoice:  true,
	level.GameMCQ:          true,
	level.GameMemoryCards:  true,
}
...
func LintLevels(levels []level.Level) []LintIssue {
	...
	for _, lvl := range levels {
		...
		if lvl.Difficulty != level.LevelEasy && lvl.Difficulty != level.LevelMedium {
		...
		for li, lesson := range lvl.Lessons {
			...
			if lesson.Component == level.EngineComponent {
	...
	if len(qaidaIDs) > 0 {
		...
		for _, lvl := range levels {
			if lvl.CourseType != level.CourseQaida || lvl.ID%certificateEvery != 0 {
				continue
			}
			hasCert := false
...
func lintLessonData(id int, where string, lesson level.Lesson) []LintIssue {
	var issues []LintIssue
...
func lintMiniGame(id int, game level.MiniGame) []LintIssue {
	var issues []LintIssue
	...
	switch game.Type {
	case level.GameTapMatch, level.GameMemoryCards:
		issues = append(issues, lintPairs(id, where, data, maxMatchPairs)...)
	case level.GameMCQ, level.GameListenChoose:
		qs := questionList(data)
		if len(qs) == 0 {
			add("no questions")
		}
		for _, q := range qs {
			...
			issues = append(issues, lintChoice(id, where, q, game.Type == level.GameListenChoose)...)
		}
	case level.GameDragDrop:
		rounds, _ := data["rounds"].([]any)
		if len(rounds) == 0 {
			add("no rounds")
		}
