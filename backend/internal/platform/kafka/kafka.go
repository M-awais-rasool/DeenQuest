...
// NewProducerAsync returns a fire-and-forget producer tuned for
...
func NewProducerAsync(brokers []string) *Producer {
	writer := &kafkago.Writer{
		Addr:                   kafkago.TCP(brokers...),
...
func (p *Producer) PublishKeyed(ctx context.Context, topic, key string, event Event) error {
...
func (p *Producer) PublishBatch(ctx context.Context, topic string, events []Event, keyFn func(Event) string) error {
	if len(events) == 0 {
		return nil
	}
	now := time.Now()
	...
	for _, e := range events {
		...
		m := kafkago.Message{Topic: topic, Value: value}
		if keyFn != nil {
			if k := keyFn(e); k != "" {
				m.Key = []byte(k)
			}
...
// Consumer wraps kafkago.Reader for consuming messages from a topic.
...
// NewConsumerWithConfig builds a reader with explicit tuning. Used by the
...
func NewConsumerWithConfig(brokers []string, topic, groupID string, cfg ConsumerConfig) *Consumer {
	if cfg.MinBytes == 0 {
		cfg.MinBytes = 1
	}
	if cfg.MaxBytes == 0 {
